...
float calculateShadow(in vec4 shadow_coord, in float bias)
{
	return (texture(u_shadowMap, shadow_coord.xy).z * bias < shadow_coord.z) ? 0 : 1;
}
...
void main()
{
	...
	for (int i = 0; i < u_numLights; i++)
	{
		float shadow = (u_lights[i].shadowCaster && ((u_material.parameters & SHADOW_MAP) != 0u)) 
			? calculateShadow(fs_in.shadow_coord, 0.001) : 1.0;
